...
import type { CuentaServicio, EstadisticasMensuales, TipoServicio } from '../../tipos';
...
export const PlantillaReporteMensual: React.FC<PlantillaReporteMensualProps> = ({
  cuentas,
  estadisticas,
  incluirGraficos = true
}) => {
  const fechaGeneracion = new Date();
  const nombreMes = format(new Date(estadisticas.año, estadisticas.mes - 1), 'MMMM yyyy', { locale: es });
  ...
  return (
    <div className="plantilla-reporte-mensual">
      ...
      <section className="resumen-ejecutivo">
        ...
        <div className="metricas-grid">
          <div className="metrica">
            ...
            <span className="metrica-valor">${estadisticas.totalGastos.toLocaleString('es-AR')}</span>
          </div>
          <div className="metrica">
            <span className="metrica-label">Promedio Mensual</span>
            <span className="metrica-valor">${estadisticas.promedioMensual.toLocaleString('es-AR')}</span>
          </div>
          <div className="metrica">
            <span className="metrica-label">Comparativa Anterior</span>
            <span className={`metrica-valor ${estadisticas.comparativaAnterior >= 0 ? 'positivo' : 'negativo'}`}>
              {estadisticas.comparativaAnterior >= 0 ? '+' : ''}{estadisticas.comparativaAnterior.toFixed(1)}%
            </span>
          </div>
          <div className="metrica">
      ...
      <section className="estado-pagos">
        ...
        <div className="pagos-grid">
          <div className="pago-item pagado">
            <span className="pago-label">Pagadas ({cuentasPagadas.length})</span>
            <span className="pago-valor">${totalPagado.toLocaleString('es-AR')}</span>
          </div>
          <div className="pago-item pendiente">
            <span className="pago-label">Pendientes ({cuentasPendientes.length})</span>
            <span className="pago-valor">${totalPendiente.toLocaleString('es-AR')}</span>
          </div>
        </div>
      </section>
      ...
      <section className="detalle-servicios">
        ...
        {Object.entries(estadisticas.gastosPorServicio).map(([servicio, total]) => {
          const tipoServicio = servicio as TipoServicio;
          const cuentasServicio = cuentasPorServicio[tipoServicio] || [];
          const porcentaje = estadisticas.totalGastos > 0 ? (total / estadisticas.totalGastos) * 100 : 0;
            ...
            <div key={servicio} className="servicio-detalle">
              <div className="servicio-header">
                ...
                <div className="servicio-totales">
                  <span className="servicio-total">${total.toLocaleString('es-AR')}</span>
                  <span className="servicio-porcentaje">({porcentaje.toFixed(1)}%)</span>
                </div>
              </div>
              ...
              <table className="cuentas-tabla">
                ...
                <tbody>
                  {cuentasServicio
                    ...
                    .map(cuenta => (
                      <tr key={cuenta.id} className={cuenta.pagada ? 'pagada' : 'pendiente'}>
                        <td>{format(cuenta.fechaVencimiento, 'dd/MM/yyyy', { locale: es })}</td>
                        <td>${cuenta.monto.toLocaleString('es-AR')}</td>
                        <td>
                          <span className={`estado-badge ${cuenta.pagada ? 'pagada' : 'pendiente'}`}>
                            {cuenta.pagada ? 'Pagada' : 'Pendiente'}
                          </span>
                        </td>
                        <td>{format(cuenta.fechaCreacion, 'dd/MM/yyyy', { locale: es })}</td>
                      </tr>
                    ))}
                </tbody>
              </table>
